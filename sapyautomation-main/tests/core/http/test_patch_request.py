...
class TestPatchRequest(unittest.TestCase):
    ...
    @unittest.skip
    def test_patch_request_no_header(self):
        r = self.patch_ob.patch_request(self.url, self.payload)
        self.assertEqual(r.status_code, 200)

    @unittest.skip
    def test_patch_request_headers(self):
        r = self.patch_ob.patch_request(self.url, self.payload, self.headers)
        self.assertEqual(r.status_code, 200)
